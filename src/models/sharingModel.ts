...
export const characterShareModel = {
  ...
  async unshareCharacter(characterId: string, ownerId: string, targetUserDisplayName: string) {
    const characterRepo = AppDataSource.getRepository(Character);
    const shareRepo = AppDataSource.getRepository(CharacterShare);
    const userRepo = AppDataSource.getRepository(User);
    ...
    // logic for deleting share entry
    const result = await shareRepo.delete({
      character: { id: characterId },
      user: { id: targetUser.id },
    });
